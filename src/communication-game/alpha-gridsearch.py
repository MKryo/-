...
mutual_info_list_eachsum = np.array([0.0]*len(np.arange(0, 1.0, 0.05)))

for sim_idx in range(SIM_NUM):
    mutual_info_list=[]
    corr_list=[]
    entropy_list=[]
    grid_alpha = np.arange(0, 1.0, 0.05)
    # grid_m = np.arange(-2.0, 2.0, 0.1)
    alpha_m_mutual_column = np.array([0,0,0])
    o=0
    m = -1.1
    for j in range(len(grid_alpha)):
        alpha = grid_alpha[j]
        # 共通設定
        K=0 # 仮説数K+1
...
plt.plot(grid_alpha, mean_mutual_info_list, marker="d", label="m=-1.1")
plt.xlabel("alpha")
